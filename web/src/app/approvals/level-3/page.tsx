...
/**
 * Level 3 Approval Page (Stories 8.7, 8.8, 8.9)
 *
 * Displays approval summary and allows Level 3 (Final) approvers to
 * approve or reject the report batch after Level 2 approval.
 */
...
import {
  getLevel3ApprovalData,
  approveLevel3,
  rejectLevel3,
} from '@/lib/api/approvals';
import type { ApprovalData } from '@/types/approval';

export default function Level3ApprovalPage() {
  ...
  const loadApprovalData = useCallback(async (id: string) => {
    try {
      ...
      const data = await getLevel3ApprovalData(id);
      setApprovalData(data);

      // Check prerequisite - Level 2 must be approved
      if (
        data.status !== 'L2_APPROVED' &&
        data.status !== 'L3_APPROVED' &&
        data.status !== 'FINAL_APPROVED' &&
        data.status !== 'L3_REJECTED' &&
        data.status !== 'FINAL_REJECTED'
      ) {
        setPrerequisiteError('Level 2 approval required first');
      }
    } catch (err) {
  ...
  const handleApprove = async () => {
    ...
    try {
      await approveLevel3(batchId);
      setSuccessMessage('Level 3 approval successful - batch complete');
      setShowApproveDialog(false);
      showToast({
        variant: 'success',
        title: 'Level 3 approval successful - batch complete',
      });
      // Refresh data
      await loadApprovalData(batchId);
      // Optionally redirect to summary
      // router.push('/approvals/history');
    } catch (err) {
  ...
  const handleReject = async () => {
    ...
    if (rejectReason.trim().length < 20) {
      setRejectError('Minimum 20 characters required');
      return;
    }
    ...
    setRejectError(null);
    try {
      await rejectLevel3(batchId, { reason: rejectReason.trim() });
      setSuccessMessage('Level 3 rejection recorded');
      setShowRejectDialog(false);
      setRejectReason('');
      showToast({ variant: 'success', title: 'Level 3 rejection recorded' });
      // Refresh data
      await loadApprovalData(batchId);
    } catch (err) {
  ...
  const isAlreadyApproved =
    approvalData?.level3Approval?.status === 'APPROVED' ||
    approvalData?.status === 'FINAL_APPROVED';
  const isAlreadyRejected =
    approvalData?.status === 'L3_REJECTED' ||
    approvalData?.status === 'FINAL_REJECTED' ||
    approvalData?.level3Approval?.status === 'REJECTED';
  const canTakeAction =
    approvalData?.status === 'L2_APPROVED' && !prerequisiteError;
  ...
  return (
    <div className="container mx-auto py-6 space-y-6">
      <h1 className="text-2xl font-bold">Level 3 (Final) Approval</h1>
      ...
      <Card>
        <CardHeader>
          <CardTitle>Final Approval Summary</CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
          ...
          {/* Level 3 Approval Info */}
          {approvalData?.level3Approval && (
            <div className="p-4 border rounded-lg bg-blue-50">
              <p className="text-sm font-medium mb-1">
                Level 3 (Final) Approval
              </p>
              <p className="text-sm text-muted-foreground">
                {approvalData.level3Approval.status} by{' '}
                {approvalData.level3Approval.approver} on{' '}
                {new Date(
                  approvalData.level3Approval.timestamp,
                ).toLocaleString()}
              </p>
            </div>
          )}
      ...
      <Dialog open={showApproveDialog} onOpenChange={setShowApproveDialog}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              This is final approval - confirm to complete the batch. Once
              approved, the report will be marked as complete.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button
      ...
      <Dialog open={showRejectDialog} onOpenChange={setShowRejectDialog}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              This will reject the batch and return it for review. Please
              provide a detailed reason for rejection (minimum 20 characters).
            </DialogDescription>
          </DialogHeader>
          <div className="py-4">
          ...
          <DialogFooter>
            ...
            <Button
              ...
              disabled={isSubmitting}
            >
              {isSubmitting ? 'Processing...' : 'Submit Final Rejection'}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
