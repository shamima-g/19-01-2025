...
/**
 * Level 1 Approval Page (Stories 8.1, 8.2, 8.3)
 *
 * Displays approval summary and allows Level 1 approvers to
 * approve or reject the report batch.
 */
...
export default function Level1ApprovalPage() {
  ...
  const loadApprovalData = useCallback(async (id: string) => {
    try {
      ...
      const data = await getLevel1ApprovalData(id);
  ...
  const handleApprove = async () => {
    ...
    try {
      await approveLevel1(batchId);
      setSuccessMessage('Level 1 approval successful');
      setShowApproveDialog(false);
      showToast({ variant: 'success', title: 'Level 1 approval successful' });
      // Refresh data
  ...
  const handleReject = async () => {
    ...
    setRejectError(null);
    try {
      await rejectLevel1(batchId, { reason: rejectReason.trim() });
      setSuccessMessage('Level 1 rejection recorded');
      setShowRejectDialog(false);
      setRejectReason('');
      showToast({ variant: 'success', title: 'Level 1 rejection recorded' });
      // Refresh data
      await loadApprovalData(batchId);
    } catch (err) {
  ...
  const isAlreadyApproved = approvalData?.level1Approval?.status === 'APPROVED';
  const isAlreadyRejected =
    approvalData?.status === 'L1_REJECTED' ||
    approvalData?.level1Approval?.status === 'REJECTED';
      ...
      <Dialog open={showApproveDialog} onOpenChange={setShowApproveDialog}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Confirm approval for this batch?
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button
